...
def vgg(input_model, batch_bool):
    ...
    layer = tf.layers.batch_normalization(layer, training=batch_bool)
    layer_third = tf.nn.relu(layer)

    layer = tf.layers.max_pooling2d(layer_third, (2, 2), (2, 2), padding='same')
    ...
    layer = tf.layers.conv2d(inputs=layer, filters=filter_count * (2 ** 2), kernel_size=(3, 3), strides=(1, 1),
                             ...
                             kernel_initializer=kernel_init)
    layer = tf.layers.batch_normalization(layer, training=batch_bool)
    layer_second = tf.nn.relu(layer)

    layer = tf.layers.max_pooling2d(layer_second, (2, 2), (2, 2), padding='same')
    ...
    layer = tf.layers.conv2d(inputs=layer, filters=filter_count * (2 ** 3), kernel_size=(3, 3), strides=(1, 1),
                             ...
                             kernel_initializer=kernel_init)
    layer = tf.layers.batch_normalization(layer, training=batch_bool)
    layer_first = tf.nn.relu(layer)
    print(layer_first.shape)
    layer = tf.layers.max_pooling2d(layer_first, (2, 2), (2, 2), padding='same')

    layer = tf.layers.conv2d(inputs=layer, filters=filter_count * (2 ** 4), kernel_size=(3, 3), strides=(1, 1),
                             padding='same',
                             kernel_initializer=kernel_init)
    layer = tf.layers.batch_normalization(layer, training=batch_bool)
    layer = tf.nn.relu(layer)
    ...
    layer = tf.layers.conv2d_transpose(inputs= layer ,filters=filter_count * (2 ** 3), kernel_size=(2, 2), strides=(2, 2),
                             padding='same',
                             kernel_initializer=kernel_init)

    layer = tf.concat([layer_first,layer],axis=3)
    print(layer.shape)
    ...
    layer = tf.layers.conv2d_transpose(inputs=layer, filters=filter_count * (2 ** 2), kernel_size=(2, 2),
                                       strides=(2, 2),
                                       padding='same',
                                       kernel_initializer=kernel_init)

    layer = tf.concat([layer_second, layer], axis=3)
    ...
    layer = tf.layers.conv2d_transpose(inputs=layer, filters=filter_count * (2 ** 1), kernel_size=(2, 2),
                                       strides=(2, 2),
                                       padding='same',
                                       kernel_initializer=kernel_init)

    layer = tf.concat([layer_third, layer], axis=3)

    layer = tf.layers.conv2d(inputs=layer, filters=filter_count * (2 ** 1), kernel_size=(3, 3), strides=(1, 1),
                             padding='same',
                             kernel_initializer=kernel_init)
    layer = tf.layers.batch_normalization(layer, training=batch_bool)
    layer = tf.nn.relu(layer)
    ...
    layer = tf.layers.conv2d_transpose(inputs=layer, filters=filter_count * (2 ** 0), kernel_size=(2, 2),
                                       strides=(2, 2),
                                       padding='same',
                                       kernel_initializer=kernel_init)

    layer = tf.concat([layer_forth, layer], axis=3)

    layer = tf.layers.conv2d(inputs=layer, filters=filter_count * (2 ** 0), kernel_size=(3, 3), strides=(1, 1),
                             padding='same',
                             kernel_initializer=kernel_init)
    layer = tf.layers.batch_normalization(layer, training=batch_bool)
    layer = tf.nn.relu(layer)
    ...
    layer = tf.layers.conv2d(inputs=layer, filters=1, kernel_size=(3, 3), strides=(1, 1),
                             padding='same',
                             kernel_initializer=kernel_init)
    layer = tf.layers.batch_normalization(layer, training=batch_bool)
    layer = tf.nn.relu(layer)
...
x_data = tf.placeholder(dtype=tf.float32, shape=(None, 224, 224, 3))
...
with tf.Session() as sess:
    ...
    for one_epoch in range(epoch):
        ...
        # correct_prediction = tf.equal(tf.argmax(model,1),tf.argmax(y_data,1))
        ...
        # loss, prob = sess.run([loss, prob], feed_dict={x_data : test_list, y_data : test_label, train_bool:False})
